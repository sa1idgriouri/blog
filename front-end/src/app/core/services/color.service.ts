...
export class ColorService {
	...
	toHex(colorName: string | undefined) {
		return this.colors.find(color => color.name == colorName)?.value;
	}

	contrastOf(hexColor: string | undefined) {
		if (hexColor == undefined)
			return undefined;

		if (hexColor.toLowerCase().startsWith("rgb"))
			hexColor = this.convertRgbToHex(hexColor);

		if (hexColor.indexOf('#') === 0)
			hexColor = hexColor.slice(1);
		else {
			var hex = this.toHex(hexColor);
			if (hex != undefined)
		...
		return brightness > 125 ? '#000000dd' : '#ffffffdd';
	}
	...
	convertRgbToHex(rgb: string): string {
		...
		const startParentheses = rgb.indexOf("(");
		const endParentheses = rgb.indexOf(")");

		if (startParentheses == -1 || endParentheses == -1)
			return rgb;

		const numbers: string[] = rgb.substring(startParentheses + 1, endParentheses).split(",");
		let hexColor: string = "#";
		numbers.forEach(numberStr => {
			let hexStr = Number.parseInt(numberStr.trim()).toString(16);
			hexColor += (hexStr.length == 1 ? "0" : "") + hexStr;
		})
